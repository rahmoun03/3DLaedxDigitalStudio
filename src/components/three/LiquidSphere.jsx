...
export default function LiquidSphere({position, name, color1, color2, color3, color4}) {
	...
	const endForHive = new THREE.Vector3(3, 1.3, 0);
	const endForNove = new THREE.Vector3(-3, 1.3, 0);

	useFrame((state, delta) => {
		uniforms.current.uTime.value += delta + 0.03;
		const t = progressRightRef.current / 100;
		if(name === 'hiveBall' && meshRef.current) {
			// const target = new THREE.Vector3(position.x, position.y, position.z);
			const target = new THREE.Vector3().lerpVectors(position, endForHive, t);
			// target.x = position.x + (endForHive.x - position.x) * t;
			console.log('hiveBall target', target);
			meshRef.current.position.lerp(target, 0.05);
		}
		if(name === 'noveBall' && meshRef.current) {
			// const target = new THREE.Vector3(position.x, position.y, position.z);
			const target = new THREE.Vector3().lerpVectors(position, endForNove, t);
			// target.x = position.x + (endForNove.x - position.x) * t;
			console.log('noveBall target', target);
			meshRef.current.position.lerp(target, 0.05);
		}
	});
